"""
SilentEye — Flask API  (api.py)
=================================
HTTP wrapper around main.py scan functions.
Electron communicates with this via localhost:5000.

Start: python api.py
       python api.py --port 5001   (custom port)

Endpoints:
  GET  /status              → engine health check
  POST /scan/file           → scan single file   { "path": "C:\\..." }
  POST /scan/folder         → scan entire folder { "path": "C:\\..." }
  POST /scan/bytes          → scan file bytes    { "name": "x.jpg", "data": "<base64>" }
  GET  /quarantine/list     → list quarantined files
  POST /quarantine/clear    → clear quarantine folder
"""
...
def _serialise(result: dict) -> dict:
    """
    Make scan result JSON-safe.
    Removes non-serialisable objects, flattens nested dicts safely.
    """
    def clean(obj):
...
def _run_full_pipeline(file_path: str) -> dict:
    """
    Run complete Stage1 → Stage2A → Stage2B → Decision pipeline.
    Falls back gracefully if ML not available.
    """
    if not PIPELINE_OK:
