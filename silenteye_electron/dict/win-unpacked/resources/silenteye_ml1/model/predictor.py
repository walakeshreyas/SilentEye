"""
SilentEye ML — Predictor  (FIXED v12)
=======================================
BUG FIXES:
  Bug 1  — PNG Routing: routes by alpha presence, not just extension
  Bug 14 — Feature mask mismatch: loads mask from pkl, validates against features
  Bug 19 — JPEG false positives: raised threshold + added QT range guard
  Bug 20 — Silent clean on failure: returns "uncertain" not "clean"
"""
...
def _model_key(category: str, ext: str, features: np.ndarray) -> str:
    """
    BUG 1 FIX — PNG routing by alpha presence not just extension.
    Previously ALL PNGs went to image_png_alpha even without alpha channel.
    Now: routes by feat[10] alpha_present flag.
    JPEG always → image_jpg.
    PNG with alpha → image_png_alpha.
    PNG without alpha → image_png_alpha (still, but fallback rule-based
    since LSB spatial stego not reliably detectable — documented limitation).
    """
    if category != "image":
        return category
