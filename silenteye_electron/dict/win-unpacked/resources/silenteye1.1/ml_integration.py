"""
SilentEye — ML Integration Bridge  (v3)
=========================================
PATH LAYOUT:
  Desktop/
    SilentEye/
      silenteye_ml1/       <- ML project (Stage 2B)
      silenteye1.1/        <- heuristic project (Stage 2A)
        ml_integration.py  <- THIS FILE

ROUTING LOGIC:
  pdf / audio / doc    -> heuristic (2A) only -> final verdict, done
  image / video / svg  -> heuristic (2A) first, then:
      CASE A: heuristic got ZERO detections
              -> run ML -> take ML result as-is (final)
              -> reason: heuristic has no layers for DCT stego etc.
                         ML is the only engine that can catch these.
      CASE B: heuristic got ONE OR MORE detections
              -> run ML -> apply decision table:
                 h=malicious + ML=any        -> malicious
                 h=any       + ML=malicious  -> malicious
                 h=suspicious + ML=suspicious -> malicious (both agree)
                 h=suspicious + ML=clean     -> suspicious
                 h=clean      + ML=suspicious -> suspicious
                 h=clean      + ML=clean     -> clean
                 ML=error/unavailable        -> heuristic only, no crash

ML SCORE WEIGHTING (Case B only):
  ML prob >= 0.75 OR hard layer hit  -> ML authoritative, score = ml_score
  ML prob 0.45-0.74                  -> combined_score = max(h_score, ml_score)
  ML prob < 0.45 / not fired         -> h_score only
"""
